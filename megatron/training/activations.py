...
try:
    ...
    import os
    HAS_XIELU_CSCS = os.environ.get("CSCS_XIELU", "false") == "true"
except ImportError:
...
class XIELU(MegatronModule):
    def __init__(self, config=None, alpha_p_init=0.8, alpha_n_init=0.8, beta=0.5, eps=-1e-6, use_xielu_cscs=True, with_vector_loads=True):
        super().__init__(config=config)
        self.config = config
        self.use_xielu_cscs = use_xielu_cscs
        if self.use_xielu_cscs and HAS_XIELU_CSCS:
            print("Using CSCS xielu :D")
